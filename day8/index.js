...
const playGame = (input) => {
    let knownLength = {2:1, 4:4, 3:7, 7:8}
    let knownNumbersTotal = 0
    for(let index =0; index < input.length; ++index) {
        let row = input[index]
        let[ , result] = row.split('|')
        let [first, second, third, fourth] = result.trim().split(' ')
        if(knownLength.hasOwnProperty(first.length)) {
            ++knownNumbersTotal
        }
        if(knownLength.hasOwnProperty(second.length)) {
            ++knownNumbersTotal
        }
        if(knownLength.hasOwnProperty(third.length)) {
            ++knownNumbersTotal
        }
        if(knownLength.hasOwnProperty(fourth.length)) {
            ++knownNumbersTotal
        }
    }
    return knownNumbersTotal
    
}

const playGamePartII = (input) => {
    const total = input.reduce((sum, value) => {
        let [test, numbers] = value.split('|')
        let digits = test.trim().split(' ').map((value) => value.trim())
        let display = numbers.trim().split(' ').map((value) => value.trim())
        console.log({sum, digits, display})
        digitMapper = mapDigits(digits)
        // sum += _.sum(display.map((digit) => {
        //     console.log({digit})
        //     _.findIndex(digitMapper,((value) => value === digit.split('').sort((a,b)=> a-b).join('')))))
        // })

        sum += display.reduce((accumulator, digit) => {
            number = _.findIndex(digitMapper, (value) => value === digit.split('').sort().join(''))
...
const mapDigits = (digits) => {
    ...
    let mapped = new Array(10).fill(-1)
    let key = _.find(digits, (value) => value.length === 2)
    mapped[1] = key
    key = _.find(digits, (value) => value.length === 4)
    mapped[4] = key
    key = _.find(digits, (value) => value.length === 3)
    mapped[7] = key
    key = _.find(digits, (value) => value.length === 7)
    mapped[8] = key

    // work on 6 segments (9, 0, 6)
    let segment6 = digits.filter((value) => value.length === 6)
    for(let index=0; index < 3; index++) {
        let number = segment6[index]
        if(isNine(number, mapped)) {
            mapped[9] = number
        } else if (isZero(number, mapped)) {
            mapped[0] = number
        } else {
            mapped[6] = number
        }
    }
    let segment5 = digits.filter((value) => value.length === 5)
    for(let index=0; index < 3; ++index) {
        let number = segment5[index]
        if(isThree(number, mapped)) {
            mapped[3] = number
        } else if(isFive(number, mapped)) {
            mapped[5] = number
        } else {
            mapped[2] = number
        }
    }
    mapped.map((value, index) => console.log({index, value}))
    return mapped
}

const isThree = (number, mapped) => {
    return hasNumber(number, mapped[7])
}

const isFive = (number, mapped) => {
    // same as 6 but 5 should be missing one
    let six = mapped[6]
    let similar = 0;
    
    for(let index = 0; index < number.length; ++index){
        if(six.indexOf(number[index]) !== -1) {
            similar++
            continue
        }
    } 
...
const isNine = (number, mapped) => {
  return hasNumber(number, mapped[7]) && hasNumber(number, mapped[4])
}

const isZero = (number, mapped) => {
    //return !hasNumber(number, mapped[4]) && hasNumber(number,mapped[7])
    return hasNumber(number, mapped[1])
}

const hasNumber = (number, checkNumber) => {
    return checkNumber.split('').reduce((exists, char) => {
        return exists && _.indexOf(number.split(''), char) !== -1
    }, true)
}
